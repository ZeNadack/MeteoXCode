...
//  Meteo
//
//  Created by Georgina Jezabel Castillo Morales on 9/27/24.
//
...
final class CityViewViewModel: ObservableObject {
    ...
    var date: String {
        return dateFormater.string(from: Date(timeIntervalSince1970: (weather.current.dt)))
    }
    ...
    var conditions: String{
        if weather.current.weather.count > 0{
            return weather.current.weather[].main
        }
        return ""
    }
    ...
    var rainChances: String{
        return String (format:"%", weather.current.dew_point)
    }
    ...
    private func getWeatherInternal(city: String, for urlString: String) {
        ...
        NetworkManager<WeatherResponse>.fetch(for: URL(string: urlString!) { (result) in
            ...
            //This will give the state of the gotten information
            switch result {
